...
| **C10** | "Graviton ARM64 Fargate: 20% cost + 30% perf (deployed)" | 🔴 CONTRADICTED | (none) | No deploy workflow uses `aarch64`/`arm64`/`graviton`/`ARM64`. Confirmed via `Grep` across `digital_fabric/.github/workflows/` and `trustdb/.github/workflows/`. "(deployed)" is **false** as of audit. Per project guidance, current deploy target is `digital-fabric-production` ECS on standard Fargate; no Graviton instance type. |
...
5. **Graviton claim is contradicted by deploy config.** Per project guidance, the production stack runs on standard `ca-central-1` Fargate. No deploy workflow uses `aarch64`/`arm64`/`graviton`. The "(deployed)" status is false. Either deploy to Graviton and capture cost/perf delta against the x86_64 baseline, or remove the claim.
